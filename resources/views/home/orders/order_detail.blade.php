...
@Section('main_content')
                        ...
                        <div class="card-header border-0 align-items-start flex-wrap">
                            <div class="order-invoice-left">
                                ...
                                @if ($order['order_status'] == 'pending')
                                    ...
                                    data-bs-target="#CancelOrderModal" >Cancel Order</button>
                                @endif
                                    ...
                                    @if ($order->delivery_man)
                                        ...
                                        <div class="media justify-content-center align-items-center">
                                            <a class="btn btn--warning m-2 print--btn w-100 mb-3 customerChatButton "
                                                ...
                                                <span id="badge-deliveryMan" class="badge badge-danger"></span>
                                            </a>
                                        </div>
                        ...
                        {{ translate('Orders') }}
                                ...
                                <div class="media justify-content-center align-items-center">
                                    ...
                                    <a class="btn btn-teal m-2 print--btn w-100 mb-3 customerChatButton "
                                        id="{{ $order['id'] }}"
                                        image="{{ $order->delivery_man != null && $order->customer->image }}"
                                        addressee="vendor" onclick="openChat(this.id)">
                                        <i class="tio-chat mr-1"></i> {{ translate('messages.chat') }}
                                        {{ translate('messages.with') }} {{ translate('messages.vendor') }}
                                        <span id="badge-vendor" class="badge badge-danger"></span>
                                    </a>
                                </div>
                            @else
    ...
    @endif
    @if ($order['order_status'] == 'pending')
        <div class="modal fade" style="z-index:10000000;" id="CancelOrderModal" data-bs-backdrop="static"
            data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        <button type="button" class="btn btn-teal"
                            onclick="cancelOrder({{ $order['id'] }})" data-bs-dismiss="modal">Yes</button>
                    </div>
                </div>
            </div>
        </div>
...
<script type="module">
    ...
    $('#chatSubmit').click((e) => {
        ...
        if (message != '') {
            ...
            if (addressee == 'deliveryMan') {
                pathToChat = 'orders/' + id + '/chats/customer_rider';
            } else {
            ...
            ///// chat counts
            var countRef = ref(database, 'orders/' + id + '/chats/counts');
            var countType;
            if (addressee == 'deliveryMan') {
                countType = 'cr_count';
            } else {
            ...
            var shouldResetCount = false;
        
                get(countRef).then((snapshot) => {
                    var countData = snapshot.val();
                    if (countData && countData[countType]) {
                        
                        if(countData[countType].for == (addressee == 'deliveryMan' ? 'rider' : 'vendor')){
                            // Increment the existing count
                            update(countRef, {
                                [countType + '/count']: countData[countType].count + 1
                            });   
                        }else{
                            shouldResetCount = true;
                        }
                    } else {
                        console.log('after getCount3');
                        shouldResetCount = true;
                    }
                    ...
                    if(shouldResetCount){
                        set(countRef, {
                            [countType]: {
                                count: 1,
                                for: (addressee == 'deliveryMan' ? 'rider' : 'vendor')
                            }
                        });
                    }
                });
    ...
    function resetSelfCount(orderId, addressee){
         ///// chat counts
            var countRef = ref(database, 'orders/' + orderId + '/chats/counts');
            var countType;
            if (addressee == 'deliveryMan') {
                countType = 'cr_count';
            } else {
                ...
                get(countRef).then((snapshot) => {
                    ...
                    if (countData && countData[countType]) {
                        ...
                        if(countData[countType].for == 'customer'){
                            set(countRef, {
                            [countType]: {
                                count: 0,
                                for: 'customer'
    ...
    function loadChat(chatId, image) {
        ...
        resetSelfCount(chatId, addressee);

        // let pathToChat = 'chat_rooms/' + chatId + '/chats';
        if (addressee == 'deliveryMan') {
            pathToChat = 'orders/' + chatId + '/chats/customer_rider';
        } else {
        ...
        onChildAdded(newMsg, (data) => {
            ...
            var creater_id = "customer_" + "{{ Auth()->id() }}";
            ...
            if (data.val().creater_id != creater_id) {
                ...
                var divData = '<div class="media media-chat">\n' +
                    ...
                    '                    <p class="other_message">' + data.val().message + '</p>\n' +
                ...
                var divData = '<div class="media media-chat media-chat-reverse">\n' +
                    '                <div class="media-body">\n' +
                    '                   <h5 class="message_title">You</h5>' + 
                    '                    <p class="message_body">' + data.val().message + '</p>\n' +
                    '                        <p class="meta"><time datetime="2023-04-04T11:58:00">' + time +
                    '</time></p>\n' +
                    '                </div>\n' +
                    '            </div>\n';
     ...
     function updateOrderCountBadge(addressee, orderId) {
    var countRef = ref(database, 'orders/' + orderId + '/chats/counts');
    var countType = addressee === 'deliveryMan' ? 'cr_count' : 'cv_count';


        onValue(countRef, (snapshot) => {
            var countData = snapshot.val();
             console.log('finding chat counts', countData)
            var count = (countData && countData[countType] && countData[countType].for == 'customer') ? countData[countType].count : 0;
            
             // Update the badge
            var badgeElement = document.getElementById('badge-' + addressee);
    ...
    $(document).ready(function() {
        ...
        updateOrderCountBadge('vendor', orderId);
        updateOrderCountBadge('deliveryMan', orderId);
        
        // toastr.success('You have a new message from Rider for Order # 100041', 'Success', {
        //     closeButton: true,
        //     progressBar: true
        // });
    });
