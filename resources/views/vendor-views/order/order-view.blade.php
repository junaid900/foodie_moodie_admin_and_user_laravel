...
@push('script_2')
    ...
    <script type="module">
        ...
        import { getDatabase, set, ref, push, child, get, update, onValue, onChildAdded } from "https://www.gstatic.com/firebasejs/9.18.0/firebase-database.js";
            ...
            if(message != ''){
                ...
                get(countRef).then((snapshot) => {
                    ...
                    if (countData && countData[countType]) {
                        ...
                        if(countData[countType].for == (addressee == 'deliveryMan' ? 'rider' : 'customer')){
                    ...
                    if(shouldResetCount){
                        set(countRef, {
                            [countType]: {
                                count: 1,
                                for: (addressee == 'deliveryMan' ? 'rider' : 'customer')
                            }
                        });
                    }
                });
                ...
                if(addressee == 'customer'){
                    ///////for setting chat notification
                var notiRef = ref(database, 'notifications/user_' + {{$order['user_id']}});
                
                set(notiRef, {
                                message: 'You have new message from vendor for order ' + {{$order['id']}},
                                read: false
                            });
                }
