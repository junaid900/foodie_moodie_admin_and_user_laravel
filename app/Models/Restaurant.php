...
class Restaurant extends Model
{
    ...
    public function scopeOrderRating($q,$r_id = ''){
        $queryString = "*,"; 
        if(empty($r_id)){
            $queryString = "";
        }
        $q->selectRaw($queryString." (select AVG(rating) from order_reviews where restaurant_id = restaurants.id LIMIT 1) as avg_rating , (select COUNT(rating) from order_reviews where restaurant_id = restaurants.id LIMIT 1) as rating_count");
    }
