...
class UserRestaurentController extends Controller
{
    ...
    
    {
        ...
        if($zone_id)
        {
            ...
            $data['restaurants'] = Restaurant::withCount('order_reviews')->withAvg('order_reviews', 'rating')->whereIn('zone_id', $zone_id)->get();
            
        }else{
            //$data['restaurants'] = Restaurant::get();
            $data['restaurants'] = Restaurant::withCount('order_reviews')->withAvg('order_reviews', 'rating')->get();
    ...
    public function category_wise(Request $request,$id)
    {
        ...
        if(!$zone_id)
        {
            $restaurants['restaurants'] =Restaurant::get();
    ...
    public function restaurent_details(Request $request, $id)
    {
        ...
        try {
          ...
          $restaurent = Http::get(site_url() . 'api/v1/restaurants/details/' . $id); //site_url is helper
            ...
            $user = Auth()->user();
            if ($user) {
                $data['isFavorite'] = Favorite::where(['entity_id' => $id, 'entity_type' => 'restaurant', 'user_id' => $user->id ?? 0])->exists();
            }
    ...
    public function favorites()
    {
        ...
        $ids = Favorite::where('user_id', $user->id)->where('entity_type', 'restaurant')->pluck('entity_id');
        $restaurants = Restaurant::withCount('order_reviews')->withAvg('order_reviews', 'rating')->whereIn('id', $ids)->get();
        //return $restaurants;
        return view('home.restaurant.favorites', compact('restaurants'));
    }
    
    public function toggleFavorite(Request $request, $restaurant)
    {
        $user = Auth()->user();
        if ($user) {
            $fav = Favorite::where(['user_id' => $user->id ?? 0, 'entity_id' => $restaurant, 'entity_type' => 'restaurant'])->first();
            if ($fav) {
                $fav->delete();
                $isFavorite = false;
            } else {
                $fav = Favorite::create(['user_id' => $user->id ?? 0, 'entity_id' => $restaurant, 'entity_type' => 'restaurant']);
                $isFavorite = true;
            }
