...
class CustomerAuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if($user){
                return  _response(0,"This email has already registered.",[]);
            }
            $user = User::where('phone',$request->phone)->first();
            if($user){
                return  _response(0,"This phone has already registered.",[]);
            }
    ...
    public function check_phone(Request $request){
        ...
        // exit;
        if(!$user){
            return _response(1,'Failed',['code'=>'failed','message'=>'user not registered.']);
        }
        if($user->phone == $request->phone){
            return _response(1,'Success',['code'=>'success','message'=>'Verify Phone Number.']);
        }
        
        $u = User::where('phone', $request->phone)->first();
        if($u){
            return _response(1,'Failed',['code'=>'failed','message'=>'Phone number is registered with other account.']);
        }
